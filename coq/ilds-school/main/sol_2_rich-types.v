...
(* the "inhabitants" of the type sig A P are pairs, built with the constructor "exists", of some x  type A *and* a proof that P x holds *)
...
(*However, despite superficial resemblnces induced by notations,  {x | x < 3} is *not* a set.  *)

(*To see why {x | x < 3} is *not* a sett (of natural numbers),  bear in mind that an "element of"  {x | x < 3}. is a pair , consisting of a natural number, and a proof that the number in question ls less-than 3 *)
...
(*make the first arguments implicit, to avoid typing redundant information*)
...
(* now write a function that returns true if n = m and false ot
herwise *)
