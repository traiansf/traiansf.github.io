...
(* also, how do we prove therems by well-dounded induction ?*)
...
the accessible part of a relation R: tgink of R as the transition relation of a transition system.
 ...
 *  apply H; clear H ; intros; auto.
   clear Hcut.    
    ...
    now exists seq, (S j).
Qed.
(* the opposite implication seems to require a different notion of infiinite sequence.  Here is an approximation. *) 
...
Variable R:  A -> A -> Prop.
Variable a : A.
Hypothesis non_Acc : ~Acc R a.


Hypothesis  non_Acc_pred_ex : forall x, ~Acc R x -> {y : A | R y x /\ ~Acc R y}.


Program Fixpoint seq(n:nat) {struct n} :
...
(* alternative way of defining functions by proof*)
