...
4. The rubric explained: F1, F3, F4 — strong vs weak
5. What examiners look for; red flags
6. The dry run: Lab 6 this week

::: notes
You have built, critiqued, and evaluated. Today is performing it under examination: how to present an AI-mediated design and how to defend it cold against the F1+F3+F4 rubric. The payload is the rubric itself, taught through strong-vs-weak answer pairs. Open into why the defense matters.
:::
...
::: notes
One breath of recap. The architect-critic work is done; the defense is where you prove it was yours. The whole course was building toward a performance you give unaided.
:::
...
From W1, and now the explicit subject:

- **F1** — read & critique any AI-generated artifact on the spot.
- **F3** — articulate why you directed AI a certain way; what you accepted or rejected.
- **F4** — defend traceability: navigate your project end to end.

Today we make each one concrete — what a passing answer sounds like, and what fails.

::: notes
First of two F1+F3+F4 mentions. Every prior week trained a piece; today the rubric is the lecture. Each F gets a strong-vs-weak pair so students hear the difference. Same wording in the close.
:::
...
::: notes
F1 is the cold-read skill. The examiner can pull any diagram — your own or a stranger's — so memorising your slice is not enough; the skill is general. The example is the wrong-multiplicity from W4. Next slide: what a passing answer sounds like.
:::
...
::: notes
The contrast pair for F1. The weak answer defers (to appearances, to AI); the strong one applies the read-order out loud. Examiners listen for the read-aloud move — it proves the skill is real, not memorised.
:::

---

# F3 — Articulate Rationale
...
::: notes
F3 is the ownership check. It cannot be answered by someone who only accepted AI output. The architect-half of the trail (the directed-design narrative) is exactly the preparation for this question.
:::
...
::: notes
The contrast pair for F3. "The AI suggested it" is the signature failing answer — it cites authority, not reason. The strong answer names the problem and the judgement. This is the directed-design narrative spoken aloud.
:::

---

# F4 — Defend Traceability
...
::: notes
F4 is the trace walk from W10. The examiner picks a node and asks you to walk to another. The audit order (forward / backward / agreement / staleness) is the preparation. A student who ran their own trace audit answers instantly.
:::
...
::: notes
The contrast pair for F4. Naming a broken link you found and fixed is the strongest possible answer — it proves you maintained the trace, not just drew it. The weak answer reveals the trace lives only in scattered files, not in the student's head.
:::
...
::: notes
The positive rubric. Examiners reward the architect-critic competence, not production values. Honesty about remaining weaknesses is a strength signal — it shows a working critical filter. A candid "this part is still thin because…" beats a glossy overclaim.
:::
...
- *"The AI said it was good."* — you delegated judgement (see W11).
- Can't read your **own** diagram.
- Can't trace a requirement to its test.
- Every answer is *"AI did it"* with no rationale.

::: notes
The failing signals. Delegating evaluation to AI is the instant-fail from W11 — the defense is unaided precisely to catch it. Not being able to read your own artifact, or trace your own project, means the work wasn't really yours. Name these so students self-check.
:::
...
- The instructor picks students **at random** and asks F1 / F3 / F4 questions.
- Informal — but every gap surfaced must be fixed before W14.

Treat it as the real thing.

::: notes
The dry-run mechanics (spec §3, Lab 6). Random selection mirrors the real defense — you cannot predict the question, so the skill must be general. The gaps it surfaces are a gift: a free rehearsal before the graded W14 defense.
:::
...
- Run your own **trace audit** (W10) — fix broken links before someone finds them.
- **Re-read every AI diagram** in your slice — could you critique it cold?
- Rehearse the **"why"** for each decision.
- Know your **defect log** cold — it's your F1 + F3 evidence.

::: notes
Concrete prep. The preparation is not memorising answers — it is doing the audit and re-reading so the answers are available. The defect log is the single best prep artifact: it already contains caught defects (F1) with rationale (F3).
:::
...
Same architect-critic loop as all semester — now applied to your own defense.
...
::: notes
Reinforces ownership, the W12 form and the course's thesis. The unaided defense is not an obstacle — it is the one place where the human's irreplaceable contribution is measured directly.
:::
...
- **W13** — open workshop / Q&A: supervised work, final adjustments.
- **W14** — final presentations + per-student cold defense (with Lab 7).

Bring your slice to the workshop with its gaps already mapped.

::: notes
Clean handoff. W13 is buffer/workshop time to close the gaps Lab 6 surfaced; W14 is the graded finale. Encourage students to arrive at W13 knowing their weak spots, so the workshop is targeted.
:::
...
- **F1** — read and critique cold; name the defect, read it aloud, give the fix.
- **F3** — tie every choice to a reason, not an authority.
- **F4** — walk the trace either direction, and name a link you fixed.

::: notes
Second and final F1+F3+F4 mention. The rubric is now explicit and rehearsable. The strong-answer patterns (read aloud, reason-not-authority, walk-and-name-a-fix) are what students take into Lab 6 and W14.
:::
...
- Next: W13 workshop, then W14 final presentations.
