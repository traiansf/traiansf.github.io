...
6. Bridge to Lab 3 + Week 6

::: notes
Week 4 pinned one class diagram; today we widen to how the whole system is carved into parts. No re-introduction — open straight into decomposition. Package and component are the load-bearing pair; object and deployment are cameos.
:::
...
In Week 4 you critiqued an AI-drawn class diagram. Today: the same loop, one level up — at the architecture.

::: notes
One breath of recap. Week 4 was a single diagram; today is the structure of the whole system.
:::
...
- **Week 4 gave you one class diagram** — `User`, `Rental`, `Bike`, `Station`, `Payment` and how they relate.
...
::: notes
The bridge from Week 4. One class diagram models a slice; a system is dozens of classes that must be grouped into parts. The act of grouping — decomposition — is today's subject and today's defect surface.
:::
...
Today is Critique at the architecture level: read an AI-drawn structure and name where it over- or under-decomposes.

::: notes
First of two Critique, Rationale, Traceability mentions. Critique anchored this week on architecture views — reading a package/component diagram cold and naming bad decomposition IS the oral-defense skill. Same wording in the close.
:::
...
::: notes
This is the Critique drill for packages. Direction and cycles are what AI gets wrong and what the eye misses. A cycle means the boundary is fake — you cannot build, test, or reason about either package alone. Drill reading the arrows aloud.
:::
...
::: notes
The Critique drill for components. The replace test is the sharpest one — a real component is swappable behind its interface. AI loves to draw boxes labelled "Service" with no interfaces at all; the read-order catches that immediately.
:::
...
::: notes
Cameo. Deployment answers "where does it run," which is a non-functional requirement (NFR) conversation (Week 2). Name it so students aren't lost when AI emits one, but don't drill it. Other structural views (composite structure, profiles) get a one-line mention only — deliberate scope boundary, spec §2.
:::
...
::: notes
Sets up the gallery — Critique applied to architecture. The payload is over- vs under-decomposition (spec §2's named Week 5 failure mode), plus invented infrastructure and bad dependency direction. These are pre-built flawed bike-sharing artifacts, walked together — a dry run of the Lab 3 defect hunt.
:::
...
::: notes
The god class scaled up to architecture. AI under-decomposes when the prompt is vague — one box "covers" the system without committing to any structure. The fix is forcing it to name separable parts. This is the architecture-level twin of Week 4's god class.
:::
...
# Your Turn: One Part or Three?

With your neighbour (60s): pick which **two of these boxes are really one part** — vote, then we critique.

**Critique (the reveal):** *"What boundary does each split earn? Which of these are really one part?"*

::: notes
Quick pair vote before the reveal. Make them commit to a boundary call out loud — common answer is the `*Service`/`*Repository` pair for one entity. Then reveal the critique: each split must justify itself by a real boundary (independent deployment, independent team, independent change). Most collapse back together.
:::
...
::: notes
The critique result — the counterpart to Week 4's "the critic simplifies." Same domain, parts that earn their place, acyclic dependencies pointing toward the domain. This is what prompt #2 in the demo converges toward.
:::
...
::: notes
This IS the Critique drill for architecture, and the rubric Lab 3 will hand teams. A repeatable read-order beats ad-hoc staring. Students internalise it for the oral defense and the Lab 3 defect hunt.
:::
...
That judgment is the architect-and-critic call, and AI can't make it for you.

::: notes
Reinforces ownership, mirroring Week 4's "the human decides what matches the domain." The defense grades this judgment — why these parts, why this grain — not the diagram's polish.
:::
...
::: notes
Week 5 has no lab of its own (labs are biweekly; Lab 3 runs in Week 6). The gallery we just walked IS the Lab 3 format in miniature. Point students at the read-order as the rubric they'll use. Keep high-level — the Lab 3 brief is its own document.
:::

---

# Next Week: Behavioral I (Week 6)

Structure is done — Week 4 (classes) and Week 5 (architecture) cover *what the parts are*.

**Week 6:** use cases and sequence diagrams — *how the parts interact over time*, and where AI fabricates messages.

::: notes
Clean handoff. Structure (Weeks 4-5) closes; behaviour (Weeks 6-7) opens. The bike-sharing domain carries forward — the same parts now exchange messages.
:::
...
- **Critique** — read an AI architecture view and named where it over- or under-decomposed.
- **Traceability** — the package/component is a structural node in the trace: requirement -> use case -> class -> part.

Rationale (why you directed AI a certain way) lands in your project narrative.

::: notes
Second and final Critique, Rationale, Traceability mention. Critique anchored, same wording as the frame. The one-line trace touch extends Week 4's "class is a structural node" up to the architecture level, keeping Traceability continuity without a dedicated segment.
:::
...
- Next lecture (Week 6): behavioral I — use cases + sequence.
